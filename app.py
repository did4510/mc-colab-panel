...
def get_drive_status():
    """Checks if Google Drive is actually mounted and accessible."""
    if os.path.exists('/content/drive/MyDrive'):
        return True, "/content/drive/MyDrive/MinecraftServer"
    return False, "/content/Minecraft-server"

# Initial check
DRIVE_MOUNTED, BASE_DIR = get_drive_status()

# Check if we are in Google Colab
IS_COLAB = os.path.exists('/content')

if IS_COLAB and not DRIVE_MOUNTED:
    try:
        from google.colab import drive
        print("   Drive not mounted, attempting mount...")
        # Note: This may hang if run in a non-interactive shell (like !python app.py)
        # But we try anyway as it's the only way.
        drive.mount('/content/drive', force_remount=False)
        DRIVE_MOUNTED, BASE_DIR = get_drive_status()
    except Exception as e:
        print(f"⚠️  Drive mount attempt failed ({e.__class__.__name__})")

if not DRIVE_MOUNTED and not IS_COLAB:
    BASE_DIR = os.path.abspath("MinecraftServer")
    print(f"🏠  Local environment detected → using: {BASE_DIR}")

os.makedirs(BASE_DIR, exist_ok=True)
os.chdir(BASE_DIR)
print(f"📁  Working dir: {BASE_DIR}")
print(f"💾  Drive status: {'Mounted ✅' if DRIVE_MOUNTED else 'Local only ⚠️'}")
...
def server_info():
    # Update drive status dynamically in case user mounted it after starting
    is_mounted, _ = get_drive_status()
    return jsonify({"version":current_version,"type":current_type,
        ...
        "drive_mounted":is_mounted,"base_dir":BASE_DIR,"java_bin":JAVA_BIN,"eula_path":eula_path})
...
def files():
    ...
    safe_path = os.path.normpath(path_query).replace("..", "")
    try:
        entries = []
        ...
        for f in sorted(os.listdir(full_path)):
            ...
            entries.append({
                "name": f,
                "path": os.path.join(safe_path, f).replace("\\", "/"),
...
def get_file():
    ...
    # Security: prevent directory traversal
    safe_name = os.path.normpath(name).replace("..", "")
    try:
        return open(safe_name, errors="replace").read()
...
def save():
    ...
    safe_name = os.path.normpath(name).replace("..", "")
    try:
        os.makedirs(os.path.dirname(safe_name), exist_ok=True)
...
def delete_file():
    ...
    safe_name = os.path.normpath(name).replace("..", "")
    try:
        if os.path.isdir(safe_name):
            shutil.rmtree(safe_name)
        else:
            os.remove(safe_name)
        return "ok"
    except Exception as e: return str(e), 500
...
def rename_file():
    ...
    # Security: prevent directory traversal
    safe_old = os.path.normpath(old).replace("..", "")
    safe_new = os.path.normpath(new).replace("..", "")
    try:
...
def upload():
    ...
    path_query = request.form.get("path", ".")
    # Security: prevent directory traversal
    safe_path = os.path.normpath(path_query).replace("..", "")
    full_path = os.path.join(safe_path, f.filename)
    try:
